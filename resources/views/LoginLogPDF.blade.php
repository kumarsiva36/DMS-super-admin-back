        ...
        <table width="100%"  cellpadding="5" cellspacing="0" class="" >
            <tr>
                ...
                <td >
                    ...
                    <strong>{{ trans('WebSite.LoginLog') }} </strong>
                    </div>
                </td>
                <td width="12%">
        ...
        <div style="margin: 5px 0;">
            @if ($request)
                ...
                @if (isset($request->staff_id) && $request->staff_id !='')
                    <strong style="background-color: #E8E8E8;color:#606060;padding:1px 3px 3px;">{{ trans('WebSite.staff') .": ".$result[0]['username'] }}</strong>
                @endif
                ...
                @if (isset($request->device) && $request->device !='')
                    <strong style="background-color: #E8E8E8;color:#606060;padding:1px 3px 3px;">{{ trans('WebSite.Device') .": ".$request->device }}</strong>
                @endif
            @endif
        </div>
    ...
    <table style="width: 100%;border-collapse: collapse;font-family: poppins,arialuni,notosansjp;"
        cellspacing="1px" class="mainTable">
            <tr style="background-color: #f0efef; color: #000000; font-weight:500; font-family: poppins,arialuni,notosansjp;">
                ...
                <td style="padding : 5px ; font-family: poppins,arialuni,notosansjp;"><strong>{{ trans('WebSite.usertype') }}</strong></td>
                <td style="padding : 5px ; font-family: poppins,arialuni,notosansjp;"><strong>{{ trans('WebSite.ipaddress') }}</strong></td>
                {{-- <td style="padding : 5px ; font-family: poppins,arialuni,notosansjp;"><strong>{{ trans('WebSite.browser') }}</strong></td> --}}
                <td style="padding : 5px ; font-family: poppins,arialuni,notosansjp;"><strong>{{ trans('WebSite.login_status') }}</strong></td>
                <td style="padding : 5px ; font-family: poppins,arialuni,notosansjp;"><strong>{{ trans('WebSite.Device') }}</strong></td>
            </tr>
            @foreach ($result as $res)
            <tr>
                <td style="padding:5px; font-family: poppins,arialuni,notosansjp; font-size: 12px;"><strong>{{$res['login_date']}}</strong></td>
                <td style="padding:5px; font-family: poppins,arialuni,notosansjp; font-size: 12px;"><strong>{{$res['company_name']}}</strong></td>
                <td style="padding:5px; font-family: poppins,arialuni,notosansjp; font-size: 12px;"><strong>{{$res['username']}}</strong></td>
                <td style="padding:5px; font-family: poppins,arialuni,notosansjp; font-size: 12px;"><strong>{{$res['login_user_type']}}</strong></td>
                <td style="padding:5px; font-family: poppins,arialuni,notosansjp; font-size: 12px;"><strong>{{$res['ipaddress']}}</strong></td>
                {{-- <td style="padding:5px; font-family: poppins,arialuni,notosansjp; font-size: 12px;"><strong>{{$res['browser_details']}}</strong></td> --}}
                <td style="padding:5px; font-family: poppins,arialuni,notosansjp; font-size: 12px;"><strong>{{$res['login_status']}}</strong></td>
                <td style="padding:5px; font-family: poppins,arialuni,notosansjp; font-size: 12px;"><strong>
                    {{ (IsJson($res['browser_details'])) ? (json_decode($res['browser_details'])->platform??"Web") :"Web"; }}
                    </strong>
                </td>
            </tr>
            @endforeach
    ...
    <footer>
        <script type="text/php">
            if (isset($pdf)) {
               ...
               $pdf->page_text(28,815, "{{ trans('WebSite.LoginLog') }}", $font, 9, array(0, 0, 0));
